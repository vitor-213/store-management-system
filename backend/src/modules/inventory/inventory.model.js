...
// ✅ CORREGIR: El middleware debe usar function declaration y next correctamente
stockMovementSchema.pre("validate", function (next) {
  // `this` se refiere al documento
  const doc = this;

  if (
    MOVEMENT_DIRECTION_MAP[doc.type] !== undefined &&
    MOVEMENT_DIRECTION_MAP[doc.type] !== null
  ) {
    doc.direction = MOVEMENT_DIRECTION_MAP[doc.type];
  }

  next(); // ✅ Llamar next() al final
});
